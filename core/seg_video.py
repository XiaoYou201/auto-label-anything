...
sam_checkpoint = "sam/sam_hq_vit_h.pth"
model_type = "vit_h"
...
def mask_video(video_frames, box_list, batch_size, save_path):
    ...
    for frame in video_frames:
        image_boxes = torch.tensor([box_list[i]], device=sam.device)
        print(image_boxes)
        temp_map = {
        ...
        i = i + 1
        if (len(batched_input_list) == batch_size or (len(video_frames)-(batch_size*1)<batch_size)):
            i_batch = i_batch + 1
            ...
            ax[0].imshow(batched_frame_list[(i_batch-1)*2])
            ...
            for mask in batched_output[0]['masks']:
                ...
                show_mask(maskArray, ax[0], random_color=True)

                #保存mask图像
                maskArray[maskArray==1] = 255
                maskArray[maskArray==0] = 1
                cv2.imwrite(common.MASKS_PATH+"\\"+str(i_batch)+".png", maskArray)
                print("mask success")
                ...
                # print('area=',area)
                binary_mask2seg.write_annotation_to_json(id=1, image_id=j, category_id=1, segmentation=polygon,
                                                         area=area, iscrowed=0, bbox=image_boxes.cpu().numpy(),
                                                         path=common.POLYGON_PATH)
                binary_mask2seg.write_annotation_to_json(id=1, image_id=j, category_id=1, segmentation=rle,
                                                  area=area, iscrowed=1 , bbox=image_boxes.cpu().numpy(),
                                                  path=common.RLE_PATH)

                #
            # ax[1][1].imshow(batched_frame_list[-4])
            # for mask in batched_output[3]['masks']:
            #     show_mask(mask.cpu().numpy(), ax[1][1], random_color=True)
            #     # get_mask(mask.cpu().numpy(), i, random_color=True);
            # # for box in image_boxes:
            # #     show_box(box.cpu().numpy(), ax[1][1])
            # ax[1][1].axis('on')
